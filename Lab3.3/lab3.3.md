...
class BadBinaryType(SemanticError):
    ...
    @property
    def message(self):
        return f'Недопустимые типы операндов: {type_to_str(self.left)} {self.op} {type_to_str
(self.right)}'
...
class BadAssignmentType(SemanticError):
    ...
    @property
    def message(self):
        return f'Несовместимые типы присваивания: {type_to_str(self.target)} := {type_to_str
(self.source)}'
...
class BadReturnType(SemanticError):
    ...
    @property
    def message(self):
        return f'Несовместимый тип return: ожидалось {type_to_str(self.expected)}, получено 
{type_to_str(self.actual)}'
...
```text
define Main(char array array args)
char array text := "hello";
...
define int Length(char array s)
int result := 5;
return result
end

define PrintCount(int value)
assert value >= 0
end
```
...
define int array MakeVector(int size)
int array result := new int[size];
...
define int Sum(int array a, int size)
int sum := 0;
int i := 0 to size - 1 do
    sum := sum + a[i]
end;
...
```shell
Ошибка (2, 10): Необъявленная переменная y
```
...
```shell
Ошибка (4, 22): Несовместимый тип 2-го аргумента функции Add: ожидалось int, получено char
```

# Вывод
В данной лабораторной работе была реализована фаза семантического анализа, которая после 
синтаксического разбора проверяет корректность программы по смысловым правилам и либо подтверждает 
корректность входных данных, либо выдаёт координату и описание первой найденной ошибки. По заданию 
требовалось научиться связывать элементы программы с их типами и областями видимости, фиксировать 
координаты потенциально проблемных мест и обеспечивать понятный результат проверки, пригодный для 
дальнейшего развития компилятора. 

В рамках индивидуального варианта для языка L1 были реализованы проверки объявленности вызываемых 
функций и используемых переменных, корректности областей видимости, совместимости типов в операциях и 
присваиваниях, а также соответствия типов фактических параметров формальным. При выполнении работы 
абстрактное синтаксическое дерево было расширено служебной информацией, а парсер доработан так, чтобы 
сохранять координаты и передавать их в последующие проверки. Семантические правила реализованы через 
локальные таблицы символов, подвешенные к узлам, соответствующим областям видимости, что соответствует 
подходу из лекций и позволяет корректно обрабатывать вложенные конструкции. 

Тестирование показало, что корректные программы проходят проверку и завершаются сообщением о 
корректности, а некорректные останавливаются на первой семантической ошибке с точной координатой и 
понятным описанием причины. Примеры проверок продемонстрировали обнаружение ошибок, связанных с 
использованием необъявленных идентификаторов и несоответствием типов при передаче аргументов, что 
подтверждает работоспособность реализованных правил. В результате был получен семантический анализатор, 
интегрированный в существующий разбор языка и обеспечивающий надёжную диагностику типовых ошибок на 
уровне смысла программы.
